        ...
        <v-layout align-center justify-center>
            <v-flex xs12 sm8 md6>
                <v-card class="elevation-12">
                    ...
                    <v-card-actions>
                        <v-spacer></v-spacer>
                        <v-btn color="teal" @click.prevent="signin" :disabled="processing || !valid">Войти</v-btn>
                    </v-card-actions>
                </v-card>
            </v-flex>
        </v-layout>
    ...
    export default {
        data() {
            return {
                ...
                valid: false,
                emailRules: [
                    (v) => !!v || 'Пожалуйста введите е-мейл', // Если пустое поле
                    (v) => /^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/.test(v) || 'Неправильный емейл' // Стандартный регуляр экспрешшн
                ],
                passwordRules: [
